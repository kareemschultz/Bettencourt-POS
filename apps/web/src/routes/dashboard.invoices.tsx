...
import { Receipt, Plus, Search, Edit2, Printer, Trash2, X, CreditCard } from "lucide-react"
...
function statusBadgeClass(status: string): string {
  switch (status) {
    ...
    case "outstanding": return "bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300"
    case "paid": return "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300"
    case "overpaid": return "bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300"
...
export default function InvoicesPage() {
  ...
  const invoices = (raw as unknown as { invoices: InvoiceRow[]; total: number }).invoices

  const createMut = useMutation(
    orpc.invoices.create.mutationOptions({
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: orpc.invoices.list.queryOptions({ input: {} }).queryKey })
        setDialogOpen(false)
        setForm(emptyForm)
  ...
  const markPaidMut = useMutation(
    orpc.invoices.markPaid.mutationOptions({
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: orpc.invoices.list.queryOptions({ input: {} }).queryKey })
        setPaymentForm({ amountPaid: "", chequeNumber: "", receiptNumber: "", datePaid: todayGY(), chequeDepositDate: "" })
  ...
  function openEdit(inv: InvoiceRow) {
    ...
    setForm({
      ...
      notes: inv.notes ?? "",
      items: Array.isArray(inv.items) ? (inv.items as LineItem[]) : [{ description: "", quantity: 1, unitPrice: 0, total: 0 }],
    })
    setDialogOpen(true)
  }
      ...
      <div className="grid gap-4 lg:grid-cols-3">
                  ...
                  invoices.map((inv) => (
                    ...
                    >
                      ...
                      <TableCell className="text-right text-xs text-muted-foreground">
                        {Number(inv.amountPaid) > 0 ? formatGYD(Number(inv.amountPaid)) : "—"}
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell className="text-xs text-muted-foreground">
                        {inv.dueDate ? new Date(inv.dueDate).toLocaleDateString("en-GY") : "—"}
                      </TableCell>
                      <TableCell>
                        <div className="flex gap-1">
                          ...
                          {canDelete && inv.status !== "cancelled" && (
                            <Button
                              ...
                              onClick={(e) => { e.stopPropagation(); deleteMut.mutate({ id: inv.id }) }}
                            >
        ...
        <div className="flex flex-col gap-3">
          {selectedInvoice ? (
            <>
              {/* Invoice detail */}
              <Card className="print-invoice-only">
                <CardHeader className="pb-3">
                  <CardTitle className="flex items-center justify-between text-base">
                    <span>{selectedInvoice.invoiceNumber}</span>
                    <div className="flex items-center gap-2">
                      <Badge className={`text-[10px] ${statusBadgeClass(selectedInvoice.status)}`}>
                ...
                <CardContent className="flex flex-col gap-3 text-sm">
                  ...
                  {selectedInvoice.dueDate && (
                    <p className="text-xs text-muted-foreground">
                      Due: {new Date(selectedInvoice.dueDate).toLocaleDateString("en-GY")}
                    </p>
                  )}
                  <Table>
                    <TableHeader>
                    ...
                    <TableBody>
                      {(Array.isArray(selectedInvoice.items) ? selectedInvoice.items as LineItem[] : []).map(
                        (item, i) => (
                          <TableRow key={i}>
                  ...
                  {Number(selectedInvoice.amountPaid) > 0 && (
                    <div className="flex justify-between text-sm text-muted-foreground">
                      <span>Paid</span>
                      <span>{formatGYD(Number(selectedInvoice.amountPaid))}</span>
                    </div>
                  )}
                  {selectedInvoice.chequeNumber && (
                    <p className="text-xs text-muted-foreground">Cheque: {selectedInvoice.chequeNumber}</p>
                  )}
                  {selectedInvoice.receiptNumber && (
                    <p className="text-xs text-muted-foreground">Receipt: {selectedInvoice.receiptNumber}</p>
                  )}
                  {selectedInvoice.notes && (
                    <p className="text-xs text-muted-foreground italic">{selectedInvoice.notes}</p>
                  )}
                  {canUpdate && (
                    <Button size="sm" variant="outline" className="gap-1 w-full" onClick={() => openEdit(selectedInvoice)}>
                      <Edit2 className="size-3" />
                      Edit Invoice
                    </Button>
                  )}
              ...
              {/* Payment panel */}
              {canUpdate && !["paid", "cancelled"].includes(selectedInvoice.status) && (
                <Card>
                  ...
                  <CardContent className="flex flex-col gap-3">
                    <div className="flex flex-col gap-1">
                      ...
                      <Input
                        type="number"
                        inputMode="decimal"
                        placeholder={String(Number(selectedInvoice.total) - Number(selectedInvoice.amountPaid))}
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <Input
                        ...
                        onChange={(e) => setPaymentForm((f) => ({ ...f, chequeNumber: e.target.value }))}
                      />
                    </div>
                    <div className="flex flex-col gap-1">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div className="flex flex-col gap-1">
                        ...
                        <Input
                          ...
                          onChange={(e) => setPaymentForm((f) => ({ ...f, datePaid: e.target.value }))}
                        />
                      </div>
                      <div className="flex flex-col gap-1">
                        ...
                        <Input
                          ...
                          onChange={(e) => setPaymentForm((f) => ({ ...f, chequeDepositDate: e.target.value }))}
                        />
                      </div>
                    </div>
                    <Button
                      ...
                      disabled={!paymentForm.amountPaid || markPaidMut.isPending}
                      className="gap-1"
                    >
