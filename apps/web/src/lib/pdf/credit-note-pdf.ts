...
// Generates a professional Blob URL HTML document for credit note printing.
...
export type CreditNotePdfRow = {
	...
	total: string | number;
	/** Either field name is accepted — route files use `appliedAmount` */
...
function buildCreditNoteHtml(
	creditNote: CreditNotePdfRow,
	logo: string,
): string {
	const items = Array.isArray(creditNote.items)
		? (creditNote.items as LineItem[])
		: [];
...
<title>${escHtml(creditNote.creditNoteNumber)} — Bettencourt's Food Inc.</title>
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .doc-type { font-size: 28px; font-weight: 800; letter-spacing: 0.12em; text-align: right; color: #34d399; }
  ...
  .issued-to-section { padding: 24px 36px; display: flex; gap: 48px; border-bottom: 1px solid #e2e8f0; }
  .issued-col { flex: 1; }
  ...
  .reason-box { margin: 0 36px 0 36px; padding: 14px 18px; background: #ecfdf5; border-left: 4px solid #059669; border-radius: 0 6px 6px 0; font-size: 12px; color: #065f46; line-height: 1.6; }
  .reason-label { font-size: 9px; font-weight: 700; letter-spacing: 0.15em; color: #059669; text-transform: uppercase; margin-bottom: 6px; }

  .items-table { width: 100%; border-collapse: collapse; margin-top: 24px; }
  .items-table thead tr { background: #f0fdf4; }
  .items-table th { padding: 10px 16px; font-size: 9px; font-weight: 700; letter-spacing: 0.1em; color: #064e3b; text-transform: uppercase; border-bottom: 2px solid #a7f3d0; text-align: left; white-space: nowrap; }
  ...
  .totals-row.credit-total { font-size: 16px; font-weight: 800; color: #059669; margin-top: 4px; padding-top: 8px; border-top: 2px solid #a7f3d0; }
  .totals-row.applied .amount { color: #0369a1; }
  .totals-row.remaining .amount { color: #059669; font-size: 14px; font-weight: 700; }
...
<div class="wrapper">
  ...
  <div class="header">
    ...
    <div>
      ...
      <div class="doc-number">${escHtml(creditNote.creditNoteNumber)}</div>
      <div class="doc-meta">Issued: ${issuedStr}</div>
      <div style="text-align:right"><span class="status-pill">${escHtml(creditNote.status.toUpperCase())}</span></div>
    </div>
  </div>

  <!-- Issued To -->
  <div class="issued-to-section">
    <div class="issued-col">
      <div class="section-label">Issued To</div>
      <div class="customer-name">${escHtml(creditNote.customerName)}</div>
      ${creditNote.originalInvoiceNumber ? `<div class="customer-sub">Re: Invoice ${escHtml(creditNote.originalInvoiceNumber)}</div>` : ""}
    </div>
    <div class="issued-col">
      <div class="section-label">Issued By</div>
      <div class="customer-name">Bettencourt's Food Inc.</div>
      <div class="customer-sub">Georgetown, Guyana</div>
    </div>
  </div>
  ...
  <div class="totals-section">
    <div class="totals-table">
      ...
      <div class="totals-row separator grand"><span class="label">Total Credit Amount</span><span class="amount">${fmtGYD(total)}</span></div>
      <div class="totals-row credit-total">
        <span class="label">Credit Total</span>
        <span class="amount">${fmtGYD(total)}</span>
      </div>
      ${applied > 0 ? `<div class="totals-row applied"><span class="label">Amount Applied</span><span class="amount">-${fmtGYD(applied)}</span></div>` : ""}
      <div class="totals-row remaining">
        <span class="label">Remaining Balance</span>
        <span class="amount">${fmtGYD(remaining)}</span>
      </div>
    </div>
  </div>

  <div class="footer">
    Bettencourt's Food Inc. &nbsp;&bull;&nbsp; Credit Note
