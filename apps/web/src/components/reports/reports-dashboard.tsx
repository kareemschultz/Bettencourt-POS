...
import { ArrowDownAZ, LayoutGrid, User, ChevronDown, ChevronRight, Download, Printer, Ban, Package, CheckCircle2, AlertTriangle, LineChart as LineChartIcon } from "lucide-react"
...
export function ReportsDashboard({
  ...
  const [productSortMode, setProductSortMode] = useState<SortMode>("department")
  const [expandedCashier, setExpandedCashier] = useState<string | null>(null)
  const [reportTab, setReportTab] = useState("overview")

  const totalRevenue = departmentTotals.reduce((s, d) => s + Number(d.revenue || 0), 0)
  const totalOrders = departmentTotals.reduce((s, d) => s + Number(d.order_count || 0), 0)
  const totalItems = departmentTotals.reduce((s, d) => s + Number(d.items_sold || 0), 0)
  ...
  const paymentChartData = paymentMethods.map((p) => ({
    name: (p.method as string || "").charAt(0).toUpperCase() + (p.method as string || "").slice(1),
  ...
  const sortedProducts = [...productSales].sort((a, b) => {
    if (productSortMode === "alphabetical") {
      return (a.product_name as string || "").localeCompare(b.product_name as string || "")
    }
    const deptCmp = (a.department as string || "").localeCompare(b.department as string || "")
  ...
  function exportCSV() {
    const rows = [
      ...
      ...cashierActivity.map((c) => [
        ...
        Number(c.orders_processed || 0) > 0 ? Math.round(Number(c.total_revenue || 0) / Number(c.orders_processed)) : 0,
      ]),
    ]
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <p className="text-2xl font-bold text-foreground">{formatGYD(totalRevenue)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
      ...
      <Tabs value={reportTab} onValueChange={setReportTab}>
        ...
        <TabsContent value="overview" className="mt-4 flex flex-col gap-6">
                ...
                {dailyChartData.length > 0 ? (
                  <ResponsiveContainer width="100%" height={250}>
                    <BarChart data={dailyChartData}>
                      ...
                      <Tooltip formatter={(value: number) => formatGYD(value)} />
            ...
            <Card>
              ...
              <CardContent>
                {paymentChartData.length > 0 ? (
                  <ResponsiveContainer width="100%" height={250}>
                    <PieChart>
                      <Pie data={paymentChartData} cx="50%" cy="50%" innerRadius={50} outerRadius={90} paddingAngle={4} dataKey="value" label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`} labelLine={false}>
          ...
          {/* Department Totals */}
          <Card>
            <CardHeader><CardTitle>Department Totals</CardTitle></CardHeader>
            <CardContent>
                ...
                <Table className="min-w-[480px]">
                  ...
                  <TableBody>
                    {departmentTotals.length === 0 ? (
                      <TableRow><TableCell colSpan={4} className="h-16 text-center">No data</TableCell></TableRow>
                    ) : (
                      <>
                        {departmentTotals.map((row, i) => (
                          <TableRow key={i}>
                            <TableCell className="font-medium">{(row.department as string) || "Uncategorized"}</TableCell>
                            <TableCell className="text-right">{String(row.order_count || 0)}</TableCell>
                            <TableCell className="text-right">{String(row.items_sold || 0)}</TableCell>
                            <TableCell className="text-right font-mono">{formatGYD(Number(row.revenue || 0))}</TableCell>
                          </TableRow>
                        ))}
                        <TableRow className="bg-muted/50 font-bold">
                          ...
                          <TableCell className="text-right font-mono">{formatGYD(totalRevenue)}</TableCell>
                        </TableRow>
                      </>
                    )}
                  </TableBody>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={Math.max(300, deptChartData.length * 40)}>
                  <BarChart data={deptChartData} layout="vertical">
                    ...
                    <YAxis dataKey="name" type="category" width={100} className="text-[10px] sm:text-xs" />
                    <Tooltip formatter={(value: number) => formatGYD(value)} />
        ...
        <TabsContent value="cashier" className="mt-4 flex flex-col gap-6">
          {/* Cashier summary chart */}
          {cashierChartData.length > 0 && (
            <Card>
              <CardHeader><CardTitle>Revenue by Cashier</CardTitle></CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={Math.max(200, cashierChartData.length * 50)}>
          ...
          {/* Cashier summary table with expandable transactions */}
          <Card>
            <CardHeader>
                ...
                <Table className="min-w-[520px]">
                  ...
                  <TableBody>
                    {cashierActivity.length === 0 ? (
                      <TableRow>
                        <TableCell colSpan={5} className="h-16 text-center">No cashier data</TableCell>
                      </TableRow>
                    ) : (
                      <>
                        {cashierActivity.map((row, i) => {
                          ...
                          const detail = cashierDetails.find((c) => c.cashier_name === cashierName)
                          ...
                          return (
                            <>
                              <TableRow
                                ...
                                onClick={() => setExpandedCashier(isExpanded ? null : cashierName)}
                              >
                                ...
                                <TableCell className="text-right font-mono">{formatGYD(revenue)}</TableCell>
                                <TableCell className="text-right font-mono">{formatGYD(avg)}</TableCell>
                              </TableRow>
                              {isExpanded && detail?.transactions && detail.transactions.length > 0 && (
                                <TableRow key={`${i}-detail`}>
                                  <TableCell colSpan={5} className="bg-muted/20 p-0">
                                    <div className="px-6 py-2">
                                      <p className="mb-2 text-xs font-semibold text-muted-foreground">Individual Transactions</p>
                                      <Table>
                                        <TableHeader>
                                          <TableRow>
                                        ...
                                        <TableBody>
                                          {detail.transactions.map((tx, ti) => (
                                            <TableRow key={ti}>
                                              <TableCell className="h-7 py-0 text-xs font-medium">{tx.order_number}</TableCell>
                                              <TableCell className="h-7 py-0 text-right text-xs">{tx.item_count}</TableCell>
                                              <TableCell className="h-7 py-0 text-right font-mono text-xs">{formatGYD(tx.total)}</TableCell>
                                              <TableCell className="h-7 py-0 text-xs capitalize">{tx.payment_method}</TableCell>
                                              <TableCell className="h-7 py-0 text-xs text-muted-foreground">
                              ...
                              {isExpanded && (!detail?.transactions || detail.transactions.length === 0) && (
                                <TableRow key={`${i}-empty`}>
                                  <TableCell colSpan={5} className="bg-muted/20 py-4 text-center text-xs text-muted-foreground">
                                    Transaction details not available for this period.
                                  </TableCell>
                                </TableRow>
                              )}
                            </>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <div className="flex items-center gap-1 rounded-lg border border-border p-0.5">
                <Button
                  variant={productSortMode === "department" ? "default" : "ghost"}
                  size="sm"
                ...
                <Button
                  variant={productSortMode === "alphabetical" ? "default" : "ghost"}
                  size="sm"
                  className="h-7 gap-1.5 px-2.5 text-xs"
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      {productSortMode === "alphabetical" && <TableHead>Department</TableHead>}
                  ...
                  <TableBody>
                    {productSales.length === 0 ? (
                      ...
                      Object.entries(productsByDept).map(([dept, products]) => {
                        const deptTotal = products.reduce((s, p) => s + Number(p.revenue || 0), 0)
                        const deptQty = products.reduce((s, p) => s + Number(p.quantity_sold || 0), 0)
                        return (
                          <>
                            ...
                            {products.map((row, i) => (
                              <TableRow key={`${dept}-${i}`}>
                                <TableCell className="pl-6 font-medium">{row.product_name as string}</TableCell>
                                <TableCell className="text-right">{String(row.quantity_sold || 0)}</TableCell>
                                <TableCell className="text-right font-mono">{formatGYD(Number(row.revenue || 0))}</TableCell>
                              </TableRow>
                            ))}
                          </>
                      ...
                      sortedProducts.map((row, i) => (
                        <TableRow key={i}>
                          <TableCell className="font-medium">{row.product_name as string}</TableCell>
                          <TableCell className="text-sm text-muted-foreground">{(row.department as string) || "-"}</TableCell>
                          <TableCell className="text-right">{String(row.quantity_sold || 0)}</TableCell>
                          <TableCell className="text-right font-mono">{formatGYD(Number(row.revenue || 0))}</TableCell>
                        </TableRow>
                      ))
...
/* Void/Refund sub-report (client fetched) */
function VoidRefundReport() {
  const { data } = useSWR(
    `/api/reports?type=voids&startDate=${new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString()}&endDate=${new Date().toISOString()}`,
    fetcher
  )
  ...
  return (
    <>
      <Card>
        <CardHeader><CardTitle>Void/Refund Summary (Last 7 Days)</CardTitle></CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {summary.length === 0 ? (
                <TableRow><TableCell colSpan={4} className="text-center py-8 text-muted-foreground">No voids or refunds</TableCell></TableRow>
              ) : summary.map((row: Record<string, unknown>, i: number) => {
                const count = Number(row.void_count || 0)
                const flagged = count > 3
                return (
                  <TableRow key={i}>
                    <TableCell className="font-medium">{row.user_name as string}</TableCell>
                    <TableCell className="text-right font-mono">{count}</TableCell>
                    <TableCell className="text-right font-mono">{fmt(Number(row.voided_total || 0))}</TableCell>
                    <TableCell>
                      {flagged ? (
      ...
      {details.length > 0 && (
        <Card>
          <CardHeader><CardTitle>Recent Void/Refund Details</CardTitle></CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {details.map((d: Record<string, unknown>, i: number) => (
                  <TableRow key={i}>
                    <TableCell className="font-mono text-xs font-medium">#{d.order_number as string}</TableCell>
                    <TableCell className="text-sm">{d.user_name as string}</TableCell>
                    <TableCell>
                      <Badge variant={d.status === "voided" ? "destructive" : "outline"} className="text-[10px]">
                        {d.status as string}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-right font-mono">{fmt(Number(d.total || 0))}</TableCell>
                    <TableCell className="text-xs text-muted-foreground">
                      {new Date(d.created_at as string).toLocaleString([], { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
...
function ProductionReport() {
  ...
  const { data } = useSWR(
    `/api/reports?type=production&startDate=${today}&endDate=${today}`,
    fetcher
  )
  ...
  return (
    <Card>
      <CardHeader><CardTitle>Production vs Sales (Today)</CardTitle></CardHeader>
      <CardContent>
        <Table>
          <TableHeader>
            <TableRow>
          ...
          <TableBody>
            {items.length === 0 ? (
              <TableRow><TableCell colSpan={6} className="text-center py-8 text-muted-foreground">No production data for today</TableCell></TableRow>
...
function WeeklyTrendReport() {
  const { data } = useSWR("/api/reports?type=weekly-trend", fetcher)
  const items = Array.isArray(data) ? data : []
  const chartData = items.map((d: Record<string, unknown>) => ({
    date: new Date(d.date as string).toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" }),
    revenue: Number(d.revenue || 0),
    orders: Number(d.orders || 0),
  }))
          ...
          {chartData.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={chartData}>
                ...
                <XAxis dataKey="date" className="text-xs" />
                <YAxis className="text-xs" tickFormatter={(v) => `$${v}`} />
                <Tooltip formatter={(value: number, name: string) => [name === "revenue" ? fmt(value) : value, name === "revenue" ? "Revenue" : "Orders"]} />
                <Legend />
                <Line type="monotone" dataKey="revenue" stroke="var(--chart-1)" strokeWidth={2} dot={{ r: 4 }} name="Revenue" />
                <Line type="monotone" dataKey="orders" stroke="var(--chart-2)" strokeWidth={2} dot={{ r: 4 }} name="Orders" yAxisId={0} />
              </LineChart>
            </ResponsiveContainer>
          ) : (
            <p className="py-12 text-center text-sm text-muted-foreground">No data for the past week</p>
          )}
        </CardContent>
      </Card>
...
function PeakHoursHeatmap() {
  const { data } = useSWR(
    `/api/reports?type=hourly-sales&startDate=${new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString()}&endDate=${new Date().toISOString()}`,
    fetcher
  )
  const items = Array.isArray(data) ? data : []
  ...
  const maxRevenue = Math.max(...items.map((h: Record<string, unknown>) => Number(h.revenue || 0)), 1)
    ...
    <Card>
      ...
      <CardContent>
        <div className="grid grid-cols-17 gap-1">
          {hours.map((hour) => {
            const match = items.find((h: Record<string, unknown>) => Number(h.hour) === hour)
            ...
            const bg = intensity === 0 ? "bg-muted" :
              intensity < 0.25 ? "bg-emerald-100 dark:bg-emerald-900/30" :
              intensity < 0.5 ? "bg-emerald-300 dark:bg-emerald-700/50" :
              intensity < 0.75 ? "bg-amber-400 dark:bg-amber-600/60" :
            ...
            return (
              ...
              >
                <span className="text-[9px] font-bold">{hour}</span>
        ...
        <div className="mt-2 flex items-center justify-end gap-2 text-[10px] text-muted-foreground">
          ...
          <span className="inline-block size-3 rounded bg-emerald-300 dark:bg-emerald-700/50" /> Medium
          <span className="inline-block size-3 rounded bg-amber-400 dark:bg-amber-600/60" /> High
          <span className="inline-block size-3 rounded bg-red-500 dark:bg-red-700/60" /> Peak
        </div>
      </CardContent>
    </Card>
  )
