...
export function DiscountDialog({ open, onClose, subtotal, onApply }: DiscountDialogProps) {
  const [mode, setMode] = useState<"percent" | "fixed">("percent")
  const [value, setValue] = useState("")
  const [reason, setReason] = useState("")

  const numericValue = Number(value) || 0
  const discountAmount = mode === "percent" ? (subtotal * numericValue) / 100 : numericValue
  const clampedDiscount = Math.min(Math.max(discountAmount, 0), subtotal)

  function handleApply() {
    if (clampedDiscount <= 0) return
    const label = mode === "percent" ? `${numericValue}% off` : `${formatGYD(clampedDiscount)} off`
      ...
      <DialogContent className="max-w-xs sm:max-w-sm">
        ...
        <div className="flex flex-col gap-4 py-2">
          ...
          <div className="flex flex-col gap-2">
            <Label>{mode === "percent" ? "Discount %" : "Discount Amount (GYD)"}</Label>
            <Input
              type="number"
              inputMode="numeric"
              value={value}
          ...
          {clampedDiscount > 0 && (
            <div className="rounded-lg bg-muted/50 p-3 text-center">
              ...
              <p className="text-xl font-bold text-destructive">-{formatGYD(clampedDiscount)}</p>
              <p className="text-xs text-muted-foreground">New total: {formatGYD(subtotal - clampedDiscount)}</p>
            </div>
          )}
        </div>
