...
:: Extract and run the PowerShell block embedded below exit /b
:: No line-continuation tricks -- the PS code is read straight from this file
powershell -ExecutionPolicy Bypass -NonInteractive -Command "$lines=[IO.File]::ReadAllLines("%~f0");$i=($lines|Select-String -SimpleMatch ':: PS_START').LineNumber;Invoke-Expression(($lines[$i..($lines.Length-1)])-join"`n")"
